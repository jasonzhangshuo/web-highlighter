...
**使用 PM2 常驻运行（可选）**  
关掉终端或 Cursor 后仍希望服务在后台跑，可用 [PM2](https://pm2.keymetrics.io/)：
...
# 在项目根目录启动
cd /path/to/web-highlighter
pm2 start ecosystem.config.cjs
```

常用命令：`pm2 list` 查看状态，`pm2 logs web-highlighter` 看日志，`pm2 stop web-highlighter` 停止，`pm2 restart web-highlighter` 重启。
